...
- 题目文件夹中应包含题目说明或简要描述。
- `main()` 函数中使用 `assert` 断言进行测试验证。
- `Solution` 类的实现与题目给出的函数签名保持一致，不要额外添加题目要求之外的变体实现（如同时提供迭代和递归两种版本，除非题目明确要求）。
- `Solution` 类的方法仅保留函数签名，方法体留空，留给用户自行实现。
- 新增题目后同步更新 `index.html`，将新题目添加到对应分类列表中，并更新总题目数。
- 编译生成的二进制文件（如 `a.out`、`solution` 等）不要提交到仓库，提交前清理或确认已加入 `.gitignore`。
